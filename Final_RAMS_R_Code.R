...
#QDA Analysis - 5 Vars
set.seed(1)
qda.fit=qda(Safety_Status~flow+velo+dia+t+material,data=dataset, subset = train)

#QDA Results
summary(qda.fit)
qda.fit
qdaclass=predict(qda.fit,test)$class
table.qda=table(test_y,qdaclass)
qdaaccuracy=sum(diag(table.qda))/sum(table.qda)
qdaaccuracy

#QDA Analysis - 4 Vars
set.seed(1)
qda.fit=qda(Safety_Status~flow+velo+dia+material,data=dataset, subset = train)
...
#QDA Analysis - 3 Vars
set.seed(1)
qda.fit=qda(Safety_Status~flow+dia+material,data=dataset, subset = train)
...
#QDA Analysis - 2 Vars
set.seed(1)
qda.fit=qda(Safety_Status~flow+dia,data=dataset, subset = train)
